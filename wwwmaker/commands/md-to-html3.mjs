...
// blog 設定ファイル
import blogConfig from './config-blog.mjs';
...
const template = ejs.compile(
  fs.readFileSync(blogConfig.srcEjsDir + 'template-blog.ejs', 'utf-8'),
  { filename: blogConfig.srcEjsDir + 'template-blog.ejs', cache: true });

// AMP HTML記事生成テンプレート
const ampTemplate = ejs.compile(
  fs.readFileSync(blogConfig.srcEjsDir + blogConfig.ampDir + 'template-blog-amp.ejs', 'utf-8'),
  { filename: blogConfig.srcEjsDir + blogConfig.ampDir + 'template-blog-amp.ejs', cache: true });
...
function toRelative(url) {
  let u = new URL(url);
  if (u.origin.match(new RegExp(blogConfig.siteDomain))) {
    u = url.replace(u.origin, '');
  }
  return u;
}

const mdDir = path.normalize(blogConfig.gitRepoDir + blogConfig.mdDir).replace(/\\/ig, '/');
...
async function updateBlogPosting(mdPath,content,blogPosting,postAttr){
  ...
  // blogIdがない場合はセットする
  if (!postAttr.blogId) {
    postAttr.blogId = blogConfig.siteUrl + uuid.v4();
  }
...
async function generateArchive(docs, archiveDate,updatedDocs) {
  ...
  // normal
  const listTemplate = ejs.compile(await fs.readFile(blogConfig.srcEjsDir + 'template-blog-list.ejs', 'utf-8'), { filename: blogConfig.srcEjsDir + 'template-blog-list.ejs', cache: true });

  // AMP
  const ampListTemplatePath = blogConfig.srcEjsDir + blogConfig.ampDir + 'template-blog-amp-list.ejs';
  const ampListTemplate = ejs.compile(await fs.readFile(ampListTemplatePath, 'utf-8'), { filename: ampListTemplatePath, cache: true });

  // normal
  const listTemplateIndex = ejs.compile(await fs.readFile(blogConfig.srcEjsDir + 'template-blog-archive-index.ejs', 'utf-8'), { filename: blogConfig.srcEjsDir + 'template-blog-archive-index.ejs', cache: true });

  // AMP
  const ampListTemplateIndexPath = blogConfig.srcEjsDir + blogConfig.ampDir + 'template-blog-amp-archive-index.ejs';
  ...
  let blogPosting = Object.assign(
    ...
    config['sf:blogConfig']['sf:blogPostingDefaults']);
  const archiveIndex = [];
  ...
  for (const [year, d] of articlesPerYear) {
    ...
    if (articlesPerYear.has(year - 1)) {
      nextURL = '/' + blogConfig.siteBlogRoot + blogConfig.archiveDir + (year - 1);
    }
    if (articlesPerYear.has(year + 1)) {
      prevURL = '/' + blogConfig.siteBlogRoot + blogConfig.archiveDir + (year + 1);
    }
    
    const blogPostingUrl = blogConfig.siteUrl + blogConfig.siteBlogRoot + blogConfig.archiveDir + year;
    blogPosting = Object.assign(blogPosting, {
      '@id': blogPostingUrl,
      'keywords': config['sf:blogConfig'].Blog.keywords,
      'about': `${year}年のアーカイブ`,
    ...
    if(!years || (years.indexOf(year) != -1)){
      ...
      await fs.outputFile(
        path + '.html', 
        listTemplate({ params: { blogPosts: d.blogPosts, blogConfig: blogConfig, URL: URL, config: config, blogPosting: blogPosting, nextURL: nextURL, prevURL: prevURL } }), 'utf-8');
  
      await fs.outputFile(
        path + '.amp.html', 
        ampListTemplate({ params: { blogPosts: d.blogPosts, blogConfig: blogConfig, URL: URL, config: config, blogPosting: blogPosting, nextURL: nextURL, prevURL: prevURL} }), 'utf-8');
      ...
      if (!first) {
        ...
        // トップページのレンダリング
        // normal
        blogPosting['@id'] = blogPosting['url'] = blogConfig.siteUrl + blogConfig.siteBlogRoot + 'index';
        blogPosting.headline = config['sf:blogConfig'].Blog.headline;
        blogPosting.description = config['sf:blogConfig'].Blog.about;
...
async function generateKeywords(docs, archiveDate,updatedDocs) {
  ...
  const ampListTemplate =  ejs.compile(await fs.readFile(blogConfig.srcEjsDir + blogConfig.ampDir + 'template-blog-amp-list.ejs', 'utf-8'), { filename: blogConfig.srcEjsDir + blogConfig.ampDir + 'template-blog-amp-list.ejs', cache: true });

  const archiveTemplate = ejs.compile(await fs.readFile(blogConfig.srcEjsDir + 'template-blog-archive-category2.ejs', 'utf-8'), { filename: blogConfig.srcEjsDir + 'template-blog-archive-category2.ejs', cache: true });

  const ampArchiveTemplate = ejs.compile(await fs.readFile(blogConfig.srcEjsDir + blogConfig.ampDir + 'template-blog-amp-archive-category2.ejs', 'utf-8'), { filename: blogConfig.srcEjsDir + blogConfig.ampDir + 'template-blog-amp-archive-category2.ejs', cache: true });
  ...
  for (const [keyword, d] of docsPerKeyword) {
    ...
    if (!done) {
      nextURL = '/' + blogConfig.siteBlogRoot + blogConfig.archiveCategoryDir + encodeURIComponent(value[0]);
    }

    if (prevKeyword) {
      prevURL = '/' + blogConfig.siteBlogRoot + blogConfig.archiveCategoryDir + prevKeyword;
    }


    const blogPostingUrl = blogConfig.siteUrl + blogConfig.siteBlogRoot + blogConfig.archiveCategoryDir + encodedKeyword;
    blogPosting = Object.assign(blogPosting, {
      'keywords': keyword,
    ...
    if(!updatedKeywords || (updatedKeywords.indexOf(keyword) != -1)){
      // normal HTML 
      await fs.outputFile(path + '.html', listTemplate({ params: { blogPosts: d.blogPosts, blogConfig: blogConfig, URL: URL, config: config, keyword: keyword, blogPosting: blogPosting, nextURL: nextURL, prevURL: prevURL} }), 'utf-8');
      prevKeyword = encodedKeyword;

      // AMP HTML
      blogPosting['@id'] = blogPostingUrl + '.amp.html';
      blogPosting.url = blogPostingUrl + '.amp';
      await fs.outputFile(path + '.amp.html', ampListTemplate({ params: { blogPosts: d.blogPosts, blogConfig: blogConfig, URL: URL, config: config, keyword: keyword, blogPosting: blogPosting, nextURL: nextURL, prevURL: prevURL } }), 'utf-8');
    }
    
    keywords.push({
      text: keyword,
      size: 15 + d.count,
      url: '/' + blogConfig.siteBlogRoot + blogConfig.archiveCategoryDir + encodedKeyword
    });
    // // sitemap用データ
    // siteMapUrls.push({
  ...
  const archiveBlogPostingUrl = blogConfig.siteUrl + blogConfig.siteBlogRoot + blogConfig.archiveCategoryDir;
  blogPosting = Object.assign(blogPosting, {
  ...
  await fs.outputFile(archiveTemplateUrl + 'index.amp.html', ampArchiveTemplate({ params: { keywords: keywords, blogConfig: blogConfig, URL: URL, config: config, blogPosting: blogPosting, nextURL: null, prevURL: null } }), 'utf-8');
...
async function generateSiteMap(docs, urls) {
  ...
  const sitemapIndex = sm.buildSitemapIndex({
    urls: [
      blogConfig.siteUrl + blogConfig.siteBlogRoot + 'sitemap.xml.gz',
      blogConfig.siteUrl + 'sitemap-root.xml',
...
async function generateAtom(docs) {
  ...
  const feed = new Feed({
    ...
    copyright: config['sf:blogConfig']['sf:copylight'],
...
function parseContent(content) {
  ...
  if (tokens[0].type == 'html' || tokens[0].type == 'paragraph') {
    ...
    if (attr && attr[1]) {
      ...
      let t = Object.assign({}, config['sf:blogConfig']['sf:blogPostingDefaults']);
...
export async function update() {
  ...
  // JSONファイルのオープン
  try {
    var entries = JSON.parse(await fs.readFile(blogConfig.entriesPath, 'utf-8'));
  } catch (e) {
    if (e.code == 'ENOENT') {
      return create();
    } else {
  ...
  for (const d of files) {
    d[1] = blogConfig.mdDir + d[1].replace(/^'(.*?)'$/, '$1') || d[1];
    console.log(d[0], d[1]);
    switch (d[0]) {
    /* 追加 */
    case 'A':
...
export async function create() {
  // すでに初期設定されているかのチェック
  try {
    let dbstat = await fs.stat(blogConfig.entriesPath);
    if (dbstat.isFile()) {
...
export async function reset() {
  // entries dbの削除
  try {
    await fs.unlink(blogConfig.entriesPath);
    // await fs.unlink(blogConfig.keywordsPath);
  } catch (e) {
    if (e.code != 'ENOENT') {
      throw e;
    }
